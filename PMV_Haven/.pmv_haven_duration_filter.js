...
(function() {
    ...
    console.log(`${LOG_PREFIX} Loaded durations from cookie: Min=${minDuration}, Max=${maxDuration}`);
    ...
    window.fetch = async function(resource, options) {
        ...
        if (targetAPIs.some(api => url.includes(api))) {
            if (opts.method === 'POST' && opts.body) {
                try {
                    ...
                    console.log(`${LOG_PREFIX} Modified range for v1 API:`, bodyObj.range);
                } catch(e) {
        ...
        if (url.includes(searchAPI)) {
            ...
            if (data && data.data && Array.isArray(data.data)) {
                ...
                console.log(`${LOG_PREFIX} Filtered search results (seconds → minutes): ${originalCount} -> ${data.data.length}`);
            }
